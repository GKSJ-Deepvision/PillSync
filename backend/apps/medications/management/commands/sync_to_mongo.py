    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            for m in meds:
                ...
                med_count += 1
            self.stdout.write(self.style.SUCCESS(f"Successfully synced {med_count} medications to 'developer' & 'medications' collections in MongoDB."))
            ...
            for r in reminders:
                ...
                rem_count += 1
            self.stdout.write(self.style.SUCCESS(f"Successfully synced {rem_count} reminders to 'reminders' collection in MongoDB."))
            ...
            for u in users:
                ...
                db["developer"].update_one({"user_id": u.id}, {"$set": {"type": "user", "user": dict(u_data)}}, upsert=True)
                user_count += 1
            self.stdout.write(self.style.SUCCESS(f"Successfully synced {user_count} users to 'users' & 'developer' collections in MongoDB."))
