...
class ReminderAPITests(APITestCase):
    ...
    def test_authenticated_user_can_mark_reminder_as_taken(self):
        self.authenticate(self.user)

        response = self.client.post(f"/api/reminders/{self.reminder.id}/taken/")

        self.assertEqual(response.status_code, 200)

        self.reminder.refresh_from_db()
        self.assertEqual(self.reminder.status, Reminder.Status.TAKEN)
        ...
        self.assertIsNotNone(history.taken_at)

    def test_user_cannot_mark_other_users_reminder_as_taken(self):
        self.authenticate(self.user)

        response = self.client.post(f"/api/reminders/{self.other_reminder.id}/taken/")

        self.assertEqual(response.status_code, 404)

    def test_cannot_mark_same_reminder_as_taken_twice(self):
        self.authenticate(self.user)

        first_response = self.client.post(f"/api/reminders/{self.reminder.id}/taken/")
        second_response = self.client.post(f"/api/reminders/{self.reminder.id}/taken/")
