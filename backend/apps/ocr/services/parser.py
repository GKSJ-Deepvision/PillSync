...
def parse_prescription_text(text: str, ocr_confidence: float, recognized_names: list[str] | None = None) -> list[MedicineExtraction]:
    medicines: list[MedicineExtraction] = []
    for raw_line in text.splitlines():
        line = " ".join(raw_line.split()).strip(" -:.")
        if not line or line.lower() in {"prescription", "medications", "medicines", "patient", "doctor", "date"}:
            continue
        ...
        if not name or not re.search(r"[A-Za-z]{3,}", name):
            continue
        if recognized_names:
            name = get_close_matches(name, recognized_names, n=1, cutoff=0.45)[0] if get_close_matches(name, recognized_names, n=1, cutoff=0.45) else name
