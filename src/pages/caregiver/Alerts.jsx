...
const Alerts = () => {
  ...
  useEffect(() => {
    patientService.fetchAlerts()
      .then(setAlerts)
      .catch((err) => setError(err.response?.data?.detail || err.message || 'Unable to load alerts.'))
      .finally(() => setLoading(false));
  }, []);

  const handleResolve = async (id) => {
    try {
      await patientService.resolveAlert(id);
      setAlerts((prev) => prev.filter((a) => a.id !== id));
    } catch (err) {
      setError(err.response?.data?.detail || err.message || 'Unable to resolve alert.');
    }
  };

  const handleResolveAll = async () => {
    await Promise.all(alerts.map((alertItem) => patientService.resolveAlert(alertItem.id)));
    setAlerts([]);
  };
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 animate-fade-in" data-testid="alerts-page">
      ...
      {loading ? <Loading text="Loading patient alerts..." /> : error ? <ErrorMessage message={error} /> : alerts.length === 0 ? (
        <EmptyState
