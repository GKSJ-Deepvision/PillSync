...
const PatientDashboard = () => {
  const { user } = useAuth();
  const [patientData, setPatientData] = useState({ adherence: { monthly: 0 }, medications: [], schedule: [] });
  const [intelligence, setIntelligence] = useState(null);

  useEffect(() => {
    patientService.fetchDashboard()
      .then(setPatientData)
      .catch(() => {});
  ...
  return (
    <div className="space-y-6">
      ...
      {intelligence && <div className="grid grid-cols-1 md:grid-cols-3 gap-4"><Card title="Missed-dose risk" subtitle="Decision support"><span className="text-2xl font-extrabold capitalize text-slate-800">{intelligence.missed_dose_risk.level}</span><p className="text-xs text-slate-500 mt-1">Score {intelligence.missed_dose_risk.score}/100</p></Card><Card title="Refill watch" subtitle="Medication quantity"><span className="text-2xl font-extrabold text-slate-800">{intelligence.refill_predictions.filter((item) => item.refill_needed).length}</span><p className="text-xs text-slate-500 mt-1">medications need review</p></Card><Card title="Pattern status" subtitle="Adherence history"><span className="text-sm font-bold text-slate-800 capitalize">{intelligence.adherence_pattern.status}</span><p className="text-[11px] text-slate-500 mt-2">{intelligence.disclaimer}</p></Card></div>}
