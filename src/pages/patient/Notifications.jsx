...
const Notifications = () => {
  ...
  useEffect(() => {
    patientService.fetchNotifications()
      .then(setNotifications)
      .catch((err) => setError(err.response?.data?.detail || err.message || 'Unable to load notifications.'))
      .finally(() => setLoading(false));
  }, []);

  const handleMarkAllRead = async () => {
    await patientService.markNotificationsRead();
  ...
  const handleClearAll = async () => {
    await Promise.all(notifications.map((notification) => patientService.deleteNotification(notification.id)));
    setNotifications([]);
  };
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6 animate-fade-in" data-testid="notifications-page">
      ...
      {loading ? <Loading text="Loading notifications..." /> : error ? <ErrorMessage message={error} /> : notifications.length === 0 ? (
        <EmptyState
          title="All Caught Up!"
