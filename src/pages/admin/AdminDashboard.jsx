...
import { MOCK_SYSTEM_STATS } from '../../data/mockData';
...
const AdminDashboard = () => {
  ...
  useEffect(() => {
    patientService.fetchUserList().then(setUsers).catch(() => {});
    patientService.fetchActivityLogs().then(setLogs).catch(() => {});
  }, []);

  const patients = users.filter((account) => account.role === 'patient');
  const caregivers = users.filter((account) => account.role === 'caregiver');
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <Card title="Total Users" subtitle="Registered on system" className="flex flex-col justify-between">
          <div className="flex items-baseline gap-2 mt-2">
            <span className="text-3xl font-extrabold text-slate-800">{users.length}</span>
        ...
        <Card title="Patients Registry" subtitle="Active trackers" className="flex flex-col justify-between">
          <div className="flex items-baseline gap-2 mt-2">
            <span className="text-3xl font-extrabold text-slate-800 text-emerald-600">{patients.length}</span>
        ...
        <Card title="Caregivers Registry" subtitle="Assigned providers" className="flex flex-col justify-between">
          <div className="flex items-baseline gap-2 mt-2">
            <span className="text-3xl font-extrabold text-slate-800 text-violet-600">{caregivers.length}</span>
            <span className="text-xs text-slate-400 font-medium">accounts</span>
          </div>
        </Card>
        <Card title="System Admins" subtitle="Security access keys" className="flex flex-col justify-between">
          <div className="flex items-baseline gap-2 mt-2">
            <span className="text-3xl font-extrabold text-slate-800 text-rose-650">{admins.length}</span>
