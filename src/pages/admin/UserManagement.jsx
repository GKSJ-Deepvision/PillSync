...
const UserManagement = () => {
  ...
  useEffect(() => {
    patientService.fetchUserList()
      .then(setUsers)
      .catch((err) => setError(err.response?.data?.detail || err.message || 'Unable to load users.'))
  ...
  const handleDisableToggle = async () => {
    ...
    const updatedUser = await patientService.toggleUserStatus(selectedUser.id);
  ...
  const handleUserUpdate = async () => {
    if (!selectedUser) return;
    const updatedUser = await patientService.updateUser(selectedUser.id, { name: editName, phone: editPhone });
    setUsers((prev) => prev.map((user) => user.id === updatedUser.id ? updatedUser : user));
    setModalOpen(false);
    setSelectedUser(null);
  };
  ...
  return (
    <div className="space-y-6 animate-fade-in" data-testid="user-management-page">
      ...
      {loading && <Loading text="Loading user registry..." />}
      ...
      <Modal
        ...
        footer={
          modalType === 'disable' ? (
            ...
            <>
              <Button variant="secondary" onClick={() => setModalOpen(false)}>Cancel</Button>
              <Button variant="primary" onClick={handleUserUpdate}>Save Changes</Button>
            </>
          ) : null
        }
      >
        ...
        {selectedUser && modalType === 'edit' && (
          <div className="space-y-4" data-testid="user-edit-modal">
            <Input label="Full Name" value={editName} onChange={(event) => setEditName(event.target.value)} required />
            <Input label="Phone Number" value={editPhone} onChange={(event) => setEditPhone(event.target.value)} />
