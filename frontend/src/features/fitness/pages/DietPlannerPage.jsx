...
export default function DietPlannerPage() {
  ...
  async function handleSave() {
    ...
    const { error: exerciseError } = await assignExercises(user.id, result.exerciseIds, {
      ageGroup,
    });
    if (exerciseError) {
      setError(
        exerciseError.message || "The diet plan was saved, but exercises could not be assigned."
      );
      return;
    }
      ...
      {result && (
        <div className="card mt-6">
          ...
          <div className="mt-3 grid gap-4 sm:grid-cols-3 lg:grid-cols-4">
            {result.exerciseIds.map((id) => {
              ...
              return (
                <div
                  key={id}
                  className="flex flex-col items-center gap-1 rounded-xl border border-ink/5 p-3 text-center"
                >
                  <ExerciseFigure
                    exerciseId={id}
                    ageGroup={ageGroup}
          ...
          <p className="mt-2 font-body text-[12px] text-ink-fog">
            This is a general-wellness suggestion, not medical advice — check with your doctor
            before making major diet changes, especially with existing conditions.
          </p>
        </div>
      )}
