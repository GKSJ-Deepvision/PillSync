...
export default function LoginPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    setError("");
    setSubmitting(true);
    const { data, error: signInError } = await signIn(form);
    setSubmitting(false);
    if (signInError) {
    ...
    if (profileError || profile?.role !== form.role) {
      await supabase.auth.signOut();
      setError(
        `This account is not registered as a ${ROLES.find((role) => role.value === form.role)?.label} account.`
      );
      return;
    }
    ...
    >
      ...
      <form onSubmit={handleSubmit} className="space-y-4" noValidate>
        ...
        <div>
          <div className="flex items-center justify-between">
            ...
            <Link
              to="/forgot-password"
              className="mb-1.5 font-body text-[13px] font-medium text-indigo hover:text-indigo-deep"
            >
              Forgot it?
            </Link>
          </div>
      ...
      >
        <svg width="18" height="18" viewBox="0 0 18 18" aria-hidden="true">
          ...
          <path
            fill="#EA4335"
            d="M9 3.58c1.32 0 2.5.46 3.44 1.35l2.58-2.58C13.46.89 11.43 0 9 0A9 9 0 0 0 .98 4.97l2.97 2.33C4.66 5.17 6.65 3.58 9 3.58Z"
          />
        </svg>
