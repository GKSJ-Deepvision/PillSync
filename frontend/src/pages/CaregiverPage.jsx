...
import { Users, AlertTriangle, CheckCircle2, HeartPulse, PhoneCall, ShieldAlert } from 'lucide-react';

const PATIENTS_DATA = [
  { id: 'p1', name: 'Alex Morgan (Self)', age: 68, adherence: '92%', status: 'Normal', missedDoses: 1, lastTaken: 'Today, 08:00 AM', conditions: ['Diabetes', 'Blood Pressure'] },
  { id: 'p2', name: 'Eleanor Morgan (Mother)', age: 84, adherence: '78%', status: 'Attention Needed', missedDoses: 3, lastTaken: 'Yesterday, 09:00 PM', conditions: ['Thyroid', 'Heart'] },
  { id: 'p3', name: 'Robert Morgan (Father)', age: 86, adherence: '95%', status: 'Normal', missedDoses: 0, lastTaken: 'Today, 08:30 AM', conditions: ['Vitamins', 'Blood Pressure'] },
];
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {patients.map((patient) => (
          <div key={patient.id} className="p-5 rounded-3xl glass-card border border-slate-200/80 dark:border-slate-800/80 space-y-4">
            <div className="flex items-start justify-between">
              <div>
                <h4 className="text-base font-bold text-slate-900 dark:text-white">{patient.name}</h4>
                <p className="text-xs text-slate-500 dark:text-slate-400">Age {patient.age} &bull; {patient.conditions.join(', ')}</p>
              </div>
            ...
            <div className="p-3 rounded-2xl bg-slate-50 dark:bg-slate-900 border border-slate-200/60 dark:border-slate-800 grid grid-cols-2 gap-2 text-xs">
              <div>
                <span className="text-slate-400 block font-medium text-[10px]">Adherence Rate</span>
                <strong className="text-slate-900 dark:text-white text-base font-bold">{patient.adherence}</strong>
              </div>
              <div>
                <span className="text-slate-400 block font-medium text-[10px]">Missed Doses</span>
                <strong className="text-rose-600 dark:text-rose-400 text-base font-bold">{patient.missedDoses}</strong>
              </div>
            </div>
