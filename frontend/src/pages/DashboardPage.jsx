...
export default function DashboardPage() {
  ...
  const handleTakeDose = (medId) => {
    setMedicines((prev) =>
      prev.map((m) => (m.id === medId ? { ...m, stock: Math.max(0, m.stock - 1) } : m))
    );
  };

  const handleMissDose = (medId) => {
    // Log missed dose
    ...
    <div className="space-y-6">
      ...
      <div className="p-6 rounded-3xl bg-gradient-to-r from-brand-600 via-indigo-600 to-brand-700 text-white shadow-xl relative overflow-hidden flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div className="space-y-1 relative z-10">
          ...
          <p className="text-xs sm:text-sm text-brand-100/90 max-w-xl">
            You have <strong className="text-white font-bold">{medicines.length} active medicine schedules</strong> today. Your adherence rate is <strong className="text-emerald-300">92% this week</strong>.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="p-5 rounded-3xl glass-card border border-slate-200/80 dark:border-slate-800/80 flex items-center justify-between">
          <div>
            <p className="text-xs text-slate-500 dark:text-slate-400 font-medium">Active Medicines</p>
            <h3 className="text-2xl font-extrabold text-slate-900 dark:text-white mt-1">{medicines.length}</h3>
        ...
        <div className="p-5 rounded-3xl glass-card border border-slate-200/80 dark:border-slate-800/80 flex items-center justify-between">
          <div>
            <p className="text-xs text-slate-500 dark:text-slate-400 font-medium">Weekly Adherence</p>
            <h3 className="text-2xl font-extrabold text-slate-900 dark:text-white mt-1">92%</h3>
            <span className="text-[11px] font-semibold text-emerald-600 dark:text-emerald-400 flex items-center gap-0.5 mt-1">
        ...
        <div className="p-5 rounded-3xl glass-card border border-slate-200/80 dark:border-slate-800/80 flex items-center justify-between">
          <div>
            <p className="text-xs text-slate-500 dark:text-slate-400 font-medium">Refill Alerts</p>
            <h3 className="text-2xl font-extrabold text-rose-600 dark:text-rose-400 mt-1">{lowStockMeds.length}</h3>
        ...
        <div className="p-5 rounded-3xl glass-card border border-slate-200/80 dark:border-slate-800/80 flex items-center justify-between">
          <div>
            <p className="text-xs text-slate-500 dark:text-slate-400 font-medium">Caregiver Sync</p>
            <h3 className="text-2xl font-extrabold text-slate-900 dark:text-white mt-1">Active</h3>
