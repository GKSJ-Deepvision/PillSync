...
import { Clock, Sun, Sunset, Moon, CheckCircle2, XCircle, BellRing, Smartphone, Mail } from 'lucide-react';

const REMINDER_SCHEDULES = [
  { id: 'rem-1', name: 'Metformin 500mg', time: '08:00 AM', period: 'Morning', status: 'taken', disease: 'Diabetes' },
  { id: 'rem-2', name: 'Amlodipine 5mg', time: '08:00 AM', period: 'Morning', status: 'taken', disease: 'Blood Pressure' },
  { id: 'rem-3', name: 'Amoxicillin 250mg', time: '01:30 PM', period: 'Afternoon', status: 'pending', disease: 'Antibiotics' },
  { id: 'rem-4', name: 'Metformin 500mg', time: '09:00 PM', period: 'Night', status: 'pending', disease: 'Diabetes' },
  { id: 'rem-5', name: 'Atorvastatin 20mg', time: '09:30 PM', period: 'Night', status: 'pending', disease: 'Heart' },
];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
            Real-time dosage notifications, daily timeslot schedules, and snooze controls.
          </p>
        </div>
      ...
      {['Morning', 'Afternoon', 'Night'].map((period) => {
        ...
        const iconColor = period === 'Morning' ? 'text-amber-500' : period === 'Afternoon' ? 'text-orange-500' : 'text-indigo-400';

        return (
          <div key={period} className="p-6 rounded-3xl glass-card border border-slate-200/80 dark:border-slate-800/80 space-y-4">
            <div className="flex items-center gap-2 pb-3 border-b border-slate-100 dark:border-slate-800">
              <Icon className={`w-5 h-5 ${iconColor}`} />
              <h3 className="text-base font-bold text-slate-900 dark:text-white">
                {period} Dosage Slot
              </h3>
              <span className="text-xs text-slate-400 font-medium">({periodReminders.length} scheduled)</span>
            </div>
              ...
              {periodReminders.map((rem) => (
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h4 className="text-sm font-bold text-slate-900 dark:text-white">{rem.name}</h4>
                      <span className="text-[11px] text-slate-500 dark:text-slate-400 font-medium">{rem.disease}</span>
                    </div>
                  </div>
